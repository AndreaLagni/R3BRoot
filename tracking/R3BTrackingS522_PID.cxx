...
/*	tree_out.SetName("tree_out");
	...
	tree_out.Branch("N_glob_tracks", &nglt);
	tree_out.Branch("N_in_tracks",   &nint);
	tree_out.Branch("N_out_tracks",  &nglt);
	...
	tree_out.Branch("v_X",    &ver_X);
	tree_out.Branch("v_Y",    &ver_Y);
	tree_out.Branch("v_Z",    &ver_Z);
	...
	tree_out.Branch("angle_X",    angle_xd,  "angle_xd[N_glob_tracks]/F");
	tree_out.Branch("angle_Y",    angle_yd,  "angle_yd[N_glob_tracks]/F");
	//tree_out.AddFriend("vertex","/feynman/home/dphn/lena/al268379/work/s522_analysis/vertex/vertex_0001.root");

	tree_out.Branch("tofd_Q", &tofdq);
...
*/
		...
		*/
		...
		AddTrackData(vec_foot, vec_PoQ, vert_foot, opan.at(fNEvents-1), tofd_Q[N_glob_tracks] , mdf_AoZ[N_glob_tracks]-0.39797333, N_glob_tracks, ToF[N_glob_tracks], FlightPath[N_glob_tracks]);
		}
		
		if(id.at(N_glob_tracks)==33){
			AddTrackData(vec_foot, vec_PoQ, vert_foot, opan.at(fNEvents-1), tofd_Q[N_glob_tracks] , mdf_AoZ[N_glob_tracks]-0.1483333, N_glob_tracks, ToF[N_glob_tracks], FlightPath[N_glob_tracks]);
...
bool R3BTrackingS522::MakeIncomingTracks()
{
	...
	for (auto & f2 : f2_hits){
		...
		for (auto & f1 : f1_hits){
			...
			N_in_tracks++;
			//if(N_in_tracks==N_glob_tracks_max) return true;
			//}vertex cond	
			//}f15
			//}f16
		}
